...
|---|--------------|-----------|------------------|
| 1 | Session state in context file | Changes every session, creates maintenance burden | SESSION_CONTEXT |
...
| 4 | Personal preferences in project | User-specific, causes team conflicts | ~/.channel/CLAUDE.md |
| 5 | Hardcoded dynamic values | Sprint/velocity/blockers become stale quickly | Project management tools |
| 6 | Session history | Context file is not a changelog | .sos/sessions/ |
...
### 1. Session State in Context File
...
**Correct**: `~/.channel/CLAUDE.md` for personal preferences, `.editorconfig` for project conventions.
...
**Why wrong**: The context file is not a changelog, grows without bound.
...
**Correct**: Use `git log --oneline -5 .channel/CLAUDE.md` for history.

---

### 9. Task Lists in Context File
...
| Personal preferences | Wrong scope | ~/.channel/CLAUDE.md |
...
Before adding content to the context file:

1. **Stale tomorrow?** -> Do not add
2. **Single session scope?** -> SESSION_CONTEXT
3. **Personal, not project-wide?** -> ~/.channel/CLAUDE.md
4. **Duplicates another source?** -> Reference, do not duplicate
5. **Harness can work without it?** -> Consider removing
...
- [boundary-test.md](boundary-test.md) - Context file validation checklist
