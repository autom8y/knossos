...
| No | **STOP. Does not belong in the context file.** |
...
> "Is the context file the authoritative source for this content?"

| Answer | Verdict |
|--------|---------|
| Yes, the context file is THE source | Proceed to Question 3 |
...
```
              ...
              v                                 v
    +--------------------+           +---------------------+
    | NOT in context     |           | Project-wide scope? |
    | file. Use SESSION_ |           +----------+----------+
    | CONTEXT or hooks   |                      |
    +--------------------+          +-----------+-----------+
                                    | NO                    | YES
...
| Personal preferences | User-specific | ~/.channel/CLAUDE.md |
...
|--------------|------------------|-----|
| Skills documentation | Context file (SYNC) | Stable, project-wide |
| Agent rite catalog | Context file (REGENERATE) | Derived from knossos |
| Workflow patterns | Context file (SYNC) | Stable infrastructure |
| Project extensions | Context file (`## Project:*`) | Satellite-owned |
...
| Personal preferences | ~/.channel/CLAUDE.md | User-scoped |
...
Before committing any context file change:

> "Is this a behavioral contract (what the harness can do and how) or transient state (what's happening now)?"

| Answer | Result |
|--------|--------|
| **Behavioral contract** | Belongs in the context file |
...
Copy this checklist before any context file modification:

```markdown
## Pre-Modification Validation

- [ ] Content passes Stability Test (accurate in 1 month)
- [ ] Content passes Source of Truth Test (context file is authoritative)
...
- [ ] Content passes Noise Test (harness needs this to work effectively)
