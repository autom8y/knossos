...
- Potnia is properly consultative (read-only)
...
- [ ] Move from doc-sre to shared:
  - `debt-ledger-template` (used by debt-triage)
  - `risk-matrix-template` (used by debt-triage)
  - `sprint-debt-packages-template` (used by debt-triage)
...
- [ ] Update references in debt-triage agents to new location
...
| observability-report | doc-sre | sre |
| reliability-plan | doc-sre | sre |
| postmortem | doc-sre | sre |
| chaos-experiment | doc-sre | sre |
| resilience-report | doc-sre | sre |

---

### P2: 10x→SRE Handoff Uses Generalized Pattern

**Decision:** When 10x-dev completes implementation and needs resilience validation, use the generalized HANDOFF artifact pattern (same as debt→hygiene).
...
- [ ] Add to pattern documentation:
  ...
  ```
- [ ] Update sre README with handoff acceptance guidance
- [ ] Update 10x-dev QA-adversary to mention sre handoff for SERVICE+ complexity
...
**For debt-triage:** After P1, update template references from `@doc-sre#*` to `@shared-templates#*`.

**For 10x-dev:** P2 adds sre validation as a handoff option after QA approval for SERVICE+ complexity.
