# lib/rite - Rite Swap Module Library

This directory contains extracted modules from `swap-rite.sh`, providing reusable infrastructure for rite resource management, transaction safety, and hook registration.

## Module Overview

The `lib/rite/` directory is the extraction target for three major refactorings (Priority 1-3) from the `SPIKE-script-code-smell-refactoring.md` analysis. These modules consolidate ~1400 lines of duplicated and complex logic into ~1432 lines of well-tested, self-contained modules.

**Purpose**: Separate infrastructure concerns from orchestration logic in `swap-rite.sh`, enabling:
- Independent testing of resource operations, transaction safety, and hook management
- Reuse across multiple rite swap scenarios
- Clearer separation between "what to do" (orchestration) and "how to do it" (infrastructure)

## Architecture

```
swap-rite.sh (orchestration)
    |
    +-- sources --> lib/rite/rite-resource.sh (resource operations)
    ...
    |                  +-- get_resource_rite()
    |
    +-- sources --> lib/rite/rite-transaction.sh (transaction safety)
    |                  |-- write_atomic()
    |                  |-- create_journal() / update_journal_*()
    |                  |-- create_staging() / stage_*() / verify_staging()
    |                  +-- create_swap_backup() / verify_backup_integrity()
    |
    +-- sources --> lib/rite/rite-hooks-registration.sh (hook registration)
                       |-- require_yq()
                       |-- parse_hooks_yaml()
...
source "$KNOSSOS_HOME/lib/rite/rite-transaction.sh"    # Transaction infrastructure first
source "$KNOSSOS_HOME/lib/rite/rite-resource.sh"       # Resource operations second
source "$KNOSSOS_HOME/lib/rite/rite-hooks-registration.sh"  # Hook registration third
```

## Modules

### rite-resource.sh (347 LOC)
...
|----------|------------|---------|-------------|
| `is_resource_from_rite` | resource_name, resource_type, find_type | 0 if from rite, 1 otherwise | Check if resource belongs to any rite |
| `get_resource_rite` | resource_name, resource_type, find_type | rite name (stdout) | Get rite name that owns a resource |
| `backup_rite_resource` | resource_type, resource_dir, marker_file, find_type | 0 on success | Backup rite-owned resources to .backup directory |
| `remove_rite_resource` | resource_type, resource_dir, marker_file, find_type | 0 on success | Remove rite-owned resources listed in marker file |
| `detect_resource_orphans` | resource_type, resource_dir, incoming_rite, find_type, glob_pattern | orphan list (stdout) | Detect orphaned resources from other rites |
...
- **Marker file pattern**: Resources tracked via `.rite-commands`, `.rite-skills`, `.rite-hooks` files
- **Logging stubs**: Provides basic logging when used standalone, overridden by swap-rite.sh

---

### rite-transaction.sh (548 LOC)

**Purpose**: Transaction infrastructure for atomic rite swaps with rollback capability.

**Extracted From**: Priority 2 refactoring (lines 100-534 in swap-rite.sh, ~300 LOC)
...
| Function | Parameters | Returns | Description |
|----------|------------|---------|-------------|
| `create_journal` | source_rite, target_rite | 0 on success, 1 if exists | Create new journal entry for swap operation |
...
| `stage_agents` | rite_name | 0 on success | Stage agents from rite |
| `stage_workflow` | rite_name | 0 on success | Stage workflow.yaml file |
...
### rite-hooks-registration.sh (537 LOC)

**Purpose**: Hook registration infrastructure for settings.local.json management.

**Extracted From**: Priority 3 refactoring (lines 2136-2565 in swap-rite.sh, ~430 LOC)
...
| `generate_hooks_json` | registrations (JSON-lines) | Claude Code hooks JSON (stdout) | Generate Claude Code hooks JSON format |
| `merge_hook_registrations` | base_registrations, rite_registrations | JSON-lines (stdout) | Merge hook registrations (base first, rite appended) |
...
| Function | Parameters | Returns | Description |
|----------|------------|---------|-------------|
| `swap_hook_registrations` | rite_name | 0 on success | Sync hook registrations to settings.local.json |
...
- **Base + rite layering**: Base hooks apply to all rites, rite hooks override/extend
...
|--------|-----------|-----|------------|
| rite-resource.sh | `tests/lib/rite/test-rite-resource.sh` | - | 14 unit tests |
| rite-transaction.sh | `tests/lib/rite/test-rite-transaction.sh` | - | 28 unit tests |
| rite-hooks-registration.sh | `tests/lib/rite/test-rite-hooks-registration.sh` | - | 22 unit tests |

### Running Tests

```bash
# Run all lib/rite tests
...
./tests/lib/rite/test-rite-resource.sh test_backup_rite_resource_commands
```
...
tests/fixtures/
  rite-resource/
    mock-rites/           # Mock rite structures
    mock-project/         # Mock .claude/ directories
  rite-transaction/
    mock-rites/
    mock-project/
  rite-hooks-registration/
    valid-hooks.yaml
    invalid-event.yaml
...
- **yq v4+**: YAML parsing (required by rite-hooks-registration.sh only)
- **find, grep, sed, awk**: Standard POSIX utilities

### Internal Dependencies

All modules require logging functions from swap-rite.sh:
...
- `KNOSSOS_HOME`: Path to roster installation (required by all modules)
- `DRY_RUN_MODE`: If set to 1, preview changes without writing (used by rite-hooks-registration.sh)
...
- `docs/refactoring/REFACTOR-rite-resource.md` - Priority 1 extraction plan
- `docs/refactoring/REFACTOR-rite-transaction.md` - Priority 2 extraction plan
- `docs/refactoring/REFACTOR-rite-hooks-registration.md` - Priority 3 extraction plan
- `docs/spikes/SPIKE-script-code-smell-refactoring.md` - Original code smell analysis

## Future Work

Potential future extractions from swap-rite.sh:
