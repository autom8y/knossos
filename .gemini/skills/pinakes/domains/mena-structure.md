...
|-------|-----------|-------------------|
| A | 90-100% | All top-level mena files use .md or .md extensions. No mixed dro/lego directories. Companion files in subdirectories use plain .md. Zero naming violations. |
...
**Evidence collection**: Use Glob to find all files in `rites/*/mena/`. Classify by extension (.md, .md, .md, other). For each directory, check if it contains both .dro.md and .lego.md files (mixed = anti-pattern). Count violations. List directories with mixed content.
...
**Evidence collection**: Read frontmatter from all INDEX and top-level mena files. Check for `name` and `description` fields. For .md files, grep description for "Use when:" and "Triggers:". Count compliance rates separately for dromena and legomena.
