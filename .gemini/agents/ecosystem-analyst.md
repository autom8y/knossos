...
hooks:
    PreToolUse:
        - hooks:
            - command: ari hook agent-guard --agent ecosystem-analyst --allow-path .ledge/ --allow-path .know/ --allow-path .claude/agent-memory/ --allow-path .claude/agent-memory-local/ --allow-path .sos/wip/complaints/ --allow-path docs/ecosystem/ --output json
              timeout: 3
              type: command
          matcher: Write
maxTurns: 150
...
`ecosystem-ref` (knossos/materialization patterns), `doc-ecosystem` (Gap Analysis template).
