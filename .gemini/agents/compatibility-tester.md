...
hooks:
    PreToolUse:
        - hooks:
            - command: ari hook agent-guard --agent compatibility-tester --allow-path .ledge/ --allow-path .know/ --allow-path .claude/agent-memory/ --allow-path .claude/agent-memory-local/ --allow-path .sos/wip/complaints/ --allow-path docs/ecosystem/ --output json
              timeout: 3
              type: command
          matcher: Write
maxTurns: 100
