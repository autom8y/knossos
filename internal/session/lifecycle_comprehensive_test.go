...
// TestEventEmitter_EmitCreated verifies creation event format.
func TestEventEmitter_EmitCreated(t *testing.T) {
	tmpDir := t.TempDir()
	eventsPath := filepath.Join(tmpDir, "events.jsonl")
	auditPath := filepath.Join(tmpDir, "audit.log")

	emitter := NewEventEmitter(eventsPath, auditPath)

	err := emitter.EmitCreated("session-20260104-160414-12345678", "Test initiative", "MODULE", "test-rite")
	if err != nil {
		t.Fatalf("EmitCreated failed: %v", err)
	}
	...
	event := events[0]
	if event.Event != EventSessionCreated {
		t.Errorf("Event type = %v, want %v", event.Event, EventSessionCreated)
	}
	if event.From != "NONE" {
		t.Errorf("From = %q, want %q", event.From, "NONE")
	}
	if event.To != "ACTIVE" {
	...
	// Verify audit file has content
	auditContent, err := os.ReadFile(auditPath)
	if err != nil {
		t.Fatalf("Read audit file failed: %v", err)
	}
	if !strings.Contains(string(auditContent), "SESSION_CREATED") {
		t.Error("Audit log should contain SESSION_CREATED")
	}
	if !strings.Contains(string(auditContent), "NONE -> ACTIVE") {
		t.Error("Audit log should contain transition")
	}
}

// TestEventEmitter_EmitParked verifies park event format.
func TestEventEmitter_EmitParked(t *testing.T) {
	tmpDir := t.TempDir()
	eventsPath := filepath.Join(tmpDir, "events.jsonl")

	emitter := NewEventEmitter(eventsPath, "")

	err := emitter.EmitParked("session-20260104-160414-12345678", "End of day", "uncommitted changes")
	if err != nil {
		t.Fatalf("EmitParked failed: %v", err)
	}
	...
	if events[0].Event != EventSessionParked {
		t.Errorf("Event type = %v, want %v", events[0].Event, EventSessionParked)
	}
	if events[0].From != "ACTIVE" {
		t.Errorf("From = %q, want %q", events[0].From, "ACTIVE")
	}
	if events[0].To != "PARKED" {
		t.Errorf("To = %q, want %q", events[0].To, "PARKED")
	}
}

// TestEventEmitter_EmitResumed verifies resume event format.
func TestEventEmitter_EmitResumed(t *testing.T) {
	tmpDir := t.TempDir()
	eventsPath := filepath.Join(tmpDir, "events.jsonl")

	emitter := NewEventEmitter(eventsPath, "")

	err := emitter.EmitResumed("session-20260104-160414-12345678")
	if err != nil {
		t.Fatalf("EmitResumed failed: %v", err)
	}
	...
	if events[0].Event != EventSessionResumed {
		t.Errorf("Event type = %v, want %v", events[0].Event, EventSessionResumed)
	}
}

// TestEventEmitter_EmitArchived verifies archive event format.
func TestEventEmitter_EmitArchived(t *testing.T) {
	tmpDir := t.TempDir()
	eventsPath := filepath.Join(tmpDir, "events.jsonl")

	emitter := NewEventEmitter(eventsPath, "")

	err := emitter.EmitArchived("session-20260104-160414-12345678", "ACTIVE")
	if err != nil {
		t.Fatalf("EmitArchived failed: %v", err)
	}
	...
	if events[0].Event != EventSessionArchived {
		t.Errorf("Event type = %v, want %v", events[0].Event, EventSessionArchived)
	}
	if events[0].From != "ACTIVE" {
		t.Errorf("From = %q, want %q", events[0].From, "ACTIVE")
	}
	if events[0].To != "ARCHIVED" {
		t.Errorf("To = %q, want %q", events[0].To, "ARCHIVED")
	}
}

// TestEventEmitter_MultipleEvents verifies event ordering in JSONL.
func TestEventEmitter_MultipleEvents(t *testing.T) {
	...
	if err := emitter.EmitCreated(sessionID, "Test", "MODULE", "test"); err != nil {
		t.Fatalf("EmitCreated failed: %v", err)
	}
	if err := emitter.EmitParked(sessionID, "EOD", "clean"); err != nil {
		t.Fatalf("EmitParked failed: %v", err)
	}
	if err := emitter.EmitResumed(sessionID); err != nil {
		t.Fatalf("EmitResumed failed: %v", err)
	}
	if err := emitter.EmitArchived(sessionID, "ACTIVE"); err != nil {
		t.Fatalf("EmitArchived failed: %v", err)
	}
	...
	for i, expected := range expectedTypes {
		if events[i].Event != expected {
			t.Errorf("Event[%d] type = %v, want %v", i, events[i].Event, expected)
		}
	}
}

// TestEventEmitter_EmptyAuditPath verifies audit is skipped when path is empty.
func TestEventEmitter_EmptyAuditPath(t *testing.T) {
	tmpDir := t.TempDir()
	eventsPath := filepath.Join(tmpDir, "events.jsonl")

	emitter := NewEventEmitter(eventsPath, "")

	// Should not fail even with empty audit path
	err := emitter.EmitCreated("session-test", "Test", "MODULE", "test")
	if err != nil {
		t.Errorf("EmitCreated with empty audit path should not fail: %v", err)
	}
}

// TestReadEvents_NonexistentFile verifies ReadEvents returns empty for missing file.
func TestReadEvents_NonexistentFile(t *testing.T) {
	events, err := ReadEvents("/tmp/nonexistent-events-12345.jsonl")
	if err != nil {
		t.Fatalf("ReadEvents should not error for missing file: %v", err)
	}
	if len(events) != 0 {
...
// TestFilterEvents verifies event filtering by type and timestamp.
func TestFilterEvents_ByType(t *testing.T) {
	events := []Event{
		{Event: EventSessionCreated, Timestamp: "2026-01-01T00:00:00Z"},
		{Event: EventSessionParked, Timestamp: "2026-01-01T01:00:00Z"},
		{Event: EventSessionResumed, Timestamp: "2026-01-01T02:00:00Z"},
		{Event: EventSessionArchived, Timestamp: "2026-01-01T03:00:00Z"},
	}

	// Filter by type
	parked := FilterEvents(events, string(EventSessionParked), time.Time{})
	if len(parked) != 1 {
		t.Errorf("Filter by PARKED: got %d events, want 1", len(parked))
	}

	// Filter by timestamp
	since, _ := time.Parse(time.RFC3339, "2026-01-01T01:30:00Z")
	recent := FilterEvents(events, "", since)
	if len(recent) != 2 {
		t.Errorf("Filter since 01:30: got %d events, want 2", len(recent))
	}

	// Filter by both
	combined := FilterEvents(events, string(EventSessionArchived), since)
	if len(combined) != 1 {
		t.Errorf("Filter ARCHIVED since 01:30: got %d events, want 1", len(combined))
	}
}
