...
func TestWrapCleansAllLockArtifacts(t *testing.T) {
	...
	for _, dir := range []string{sessionDir, locksDir, harnessMapDir, auditDir} {
...
`
	...
	// Pre-create a harness map entry pointing to this session
	if err := os.WriteFile(filepath.Join(harnessMapDir, "cc-test-session-123"), []byte(sessionID), 0644); err != nil {
		t.Fatalf("Failed to write harness map entry: %v", err)
	}
	// Also create a harness map entry pointing to a DIFFERENT session (should NOT be removed)
	if err := os.WriteFile(filepath.Join(harnessMapDir, "cc-other-session-456"), []byte("session-other-123"), 0644); err != nil {
		t.Fatalf("Failed to write other harness map entry: %v", err)
	}
	...
	// Verify: harness map entry for this session should be removed
	mapEntryPath := filepath.Join(harnessMapDir, "cc-test-session-123")
	if _, statErr := os.Stat(mapEntryPath); !os.IsNotExist(statErr) {
		t.Error("Harness map entry for wrapped session should have been removed, but still exists")
	}

	// Verify: harness map entry for OTHER session should still exist
	otherMapPath := filepath.Join(harnessMapDir, "cc-other-session-456")
	if _, statErr := os.Stat(otherMapPath); os.IsNotExist(statErr) {
		t.Error("Harness map entry for unrelated session should NOT have been removed")
	}
}
