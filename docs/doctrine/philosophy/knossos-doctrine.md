...
The repository currently named `roster` will become `knossos` when it proves itself capable of self-hosting. The `/roster/` repository IS Knossos—the SOURCE that materializes `.claude/` projections into individual projects. The name is certain; only the repository rename awaits its conditions.
